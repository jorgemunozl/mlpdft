...
def parse_qe_output(qe_output_file, dump_file):
    ...
    flargtr = 0
    printflag=[0,0,0,0]
    xdis = 8.16 #dimension x
    ydis = 8.16 #dimension y
    zdis = 8.16 #dimension z
    energy = 0.0
    framestoc = 2000 #frames between samples
    tatoms = 64 #number of atoms
    ...
    lattice_matrix = [[xdis, 0.0, 0.0], [0.0, ydis, 0.0], [0.0, 0.0, zdis]]
    types_matrix = []
        ...
        #time.sleep(0.05)
            ...
            if flagts == 1:
                if temp >= 1:
                    an = line.strip()
                    ant = an.split()
                    stress_matrix[(temp-1)][0] = float(ant[3])*1000#from kbar to bar
                    stress_matrix[(temp-1)][1] = float(ant[4])*1000
                    stress_matrix[(temp-1)][2] = float(ant[5])*1000
                temp = temp + 1
                if temp >= 4:
                    temp = 0
                    flagts = 0
...
if __name__ == "__main__":
    qe_output_file = "LiF64_kjpaw.out"
    # FitSNAP expects: dataPath/<GROUP>/*.json — LiF64-NEWJSON.in uses group DEFAULT.
    fitsnap_file = "NEWJSON/DEFAULT/output"

    os.makedirs(os.path.dirname(fitsnap_file) or ".", exist_ok=True)
